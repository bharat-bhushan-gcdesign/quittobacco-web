                                ...
                                <div class="card-body m-t-35">
                                    <div class="row">
                                        <div class="col-sm-3">  <div class="btn-group show-hide">
                                              ...
                                              <a data-toggle="tooltip" data-placement="top" title="Add"  class="btn btn-primary"  href="{{url('subscription_mangement/create')}}" > <i class="fa fa-plus"></i> </a>
                                   ...
                                   <div class=" m-t-15">
                                    <table class="table table-striped table-bordered table_res toggle_class" id="sample_6">
                                        ...
                                        <tbody>
                                          @foreach($quitReason as $k=>$val)
                                            <tr>
                                                <td>{{$k+1}}</td>
                                                <td> {{$val->title}}</td>
                                                 <td> <?php $datas=trim(html_entity_decode(strip_tags(base64_decode($val->description))))?>
                                                 {!!str_limit($datas,25)!!}
                                                </td>
                                                <td> {{$val->price}} </td>
                                                ...
                                                <td class="nowrap">
                                                <a data-toggle="tooltip" data-placement="top" title="Edit" href="{{url('subscription_mangement/edit',['id' => $val->code])}}" class="btn btn-primary btn-xs">
                                                <i class="fa fa-pencil"></i>
                                                </a>
                                                <a data-toggle="tooltip" data-placement="top" title="View" href="{{url('subscription_mangement/view',['id' => $val->code])}}" class="btn btn-info btn-xs">
                                                <i class="fa fa-eye"></i>
                                                </a>
                                                 <!-- <a href="#" class="btn btn-danger btn-xs waves-effect waves-light remove-record" data-toggle="modal" data-url="{{URL::to('/cmsdestory/'.$val->id) }}" data-id="{{$val->id}}" data-target="#custom-width-modal" title="Delete" data-placement="top">
    ...
    $(".successmsg").hide()
     ...
     $('body').on('change', '.statuschange', function() {
      ...
      var status = $(this).prop('checked');
        $.ajax({
        type : "POST",
        url : "{{ url('/') }}/subscription_mangement/statusupdate",
