                ...
                <div class="col-12 data_tables">
                    ...
                    <div class=" m-t-15">
                        <table class="table table-striped table-bordered table_res toggle_class" id="sample_6">
                            ...
                            <tbody>
                                @foreach($feedback as $k=> $feedback)
                                    <tr>
                                        <td> {{$k+1}}</td>
                                        <td> {{$feedback->email}}</td>
                                        <td> {{$feedback->user->name}}</td>
                                        <td> {{$feedback->message}}</td>
                                        <td style="display:none;" id="statusval_{{$feedback->id}}">{{$feedback->status}}</td>
                                        <td> <input type="checkbox" data-toggle="toggle" data-on="Active" data-off="In Active" data-onstyle="success" data-offstyle="danger" data-size="mini" class="statuschange" id="{{$feedback->id}}" @if($feedback->status ==1) checked @endif></td>
                                        <td class="nowrap">
                                            <a  data-toggle="tooltip" data-placement="top" title="View" href="{{route('feedback.show',['feedback' => $feedback->code])}}" class="btn btn-info btn-xs"><i class="fa fa-eye"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
@include('footer')
    ...
    $(document).ready(function(){
        ...
        $('body').on('change', '.statuschange', function() {
            ...
            $.ajax({
                type : "POST",
                url : "{{ url('/') }}/feedback/update-status/",
